...
for i in range(2, 51):
    ...
    lenBiggestGoodware = get_dimension_biggest_bytecode(goodware_bytecodes_filtered_with_Divisible_Index_Kernel)
    lenBiggestMalware = get_dimension_biggest_bytecode(malware_bytecodes_filtered_with_Divisible_Index_Kernel)
    ...
    if (lenBiggestGoodware > lenBiggestMalware):
        ...
        goodware_bytecodes_filtered_with_Divisible_Index_Kernel_zero_padding = Parallel(n_jobs=NUM_CORES)(delayed(pad_bytecode)(b, lenDef) for b in goodware_bytecodes_filtered_with_Divisible_Index_Kernel)
        malware_bytecodes_filtered_with_Divisible_Index_Kernel_zero_padding = Parallel(n_jobs=NUM_CORES)(delayed(pad_bytecode)(b, lenDef) for b in malware_bytecodes_filtered_with_Divisible_Index_Kernel)
    else:
    ...
    malware_bytecodes_filtered_with_Divisible_Index_Kernel_zero_padding.clear()
    gc.collect()
    end = time.time()
