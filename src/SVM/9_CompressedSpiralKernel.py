...
for i in range(10, 101, 3):
    ...
    start = time.time()
    compressedSpiralKernel = CompressedSpiralKernel(block=i)


    def filter(b):
        result = compressedSpiralKernel(b)
        gc.collect()
        return result
    ...
    if (lenBiggestGoodware > lenBiggestMalware):
        ...
        goodware_bytecodes_filtered_with_Compressed_Spiral_Kernel_zero_padding = Parallel(n_jobs=NUM_CORES)(delayed(pad_bytecode)(b, lenDef) for b in goodware_bytecodes_filtered_with_Compressed_Spiral_Kernel)
        malware_bytecodes_filtered_with_Compressed_Spiral_Kernel_zero_padding = Parallel(n_jobs=NUM_CORES)(delayed(pad_bytecode)(b, lenDef) for b in malware_bytecodes_filtered_with_Compressed_Spiral_Kernel)
    else:
    ...
    malware_bytecodes_filtered_with_Compressed_Spiral_Kernel_zero_padding.clear()
    gc.collect()
    end = time.time()
